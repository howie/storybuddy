...
- **TR-001**: 語音錄製 MUST 使用以下格式：44.1kHz 取樣率、16-bit 位元深度、WAV 或 AAC 編碼
- **TR-002**: 語音樣本 MUST 儲存於雲端，本地僅保留加密快取
- **TR-003**: 系統 MUST 在錄音前請求麥克風權限，權限被拒時顯示引導說明
- **TR-004**: 錄音時 MUST 顯示即時波形圖和計時器
- **TR-005**: 背景噪音超過 60dB 時 MUST 顯示提示「環境太吵，請找安靜的地方錄音」
- **TR-006**: 聲音模型 MUST 支援以下狀態：`pending`（等待處理）、`processing`（建立中）、`ready`（可使用）、`failed`（失敗）
- **TR-007**: 聲音模型建立時間 SHOULD 不超過 60 秒
- **TR-008**: 當聲音克隆失敗時，系統 MUST 提供「重新錄製」選項，並可選擇使用預設聲音繼續
- **TR-009**: 語音識別 MUST 支援 3-10 歲兒童的中文語音，識別準確率達 80% 以上
- **TR-010**: 當語音無法識別時，系統 MUST 提示「我沒聽清楚，可以再說一次嗎？」最多重試 3 次
- **TR-011**: 系統 SHOULD 支援普通話及台灣國語口音
- **TR-012**: 故事播放 MUST 支援背景播放（螢幕關閉、App 切換至背景）
- **TR-013**: 當播放被來電或通知中斷時，系統 MUST 自動暫停，中斷結束後可手動繼續
- **TR-014**: 生成的語音 MUST 使用 MP3 格式（128kbps）以平衡品質與檔案大小
...
- **PS-001**: 語音資料 MUST 使用 AES-256 加密儲存（靜態加密）
- **PS-002**: 語音資料傳輸 MUST 使用 TLS 1.3（傳輸加密）
- **PS-003**: 首次錄音前 MUST 取得用戶明確同意使用聲音進行 AI 克隆
- **PS-004**: 用戶 MUST 可隨時刪除其聲音模型及所有相關資料
- **PS-005**: 聲音資料 MUST 在用戶刪除帳號後 30 天內完全清除
