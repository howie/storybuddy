...
class TestVoicePreferencesAPI:
    ...
    async def test_set_global_preference(self, client: AsyncClient) -> None:
        ...
        p_res = await client.post(
            "/api/v1/parents", json={"id": user_id, "name": "PrefUser", "email": "pref@test.com"}
        )
        ...
        async with get_db_connection() as db:
            await db.execute(
                "INSERT OR IGNORE INTO voice_kits (id, name, provider, version) VALUES ('builtin', 'Built-in', 'azure', '1.0.0')"
            )
            await db.execute(
        ...
        response = await client.post(
            "/api/voices/preferences", json={"user_id": user_id, "default_voice_id": voice_id}
        )
    ...
    async def test_story_voice_mapping(self, client: AsyncClient) -> None:
        ...
        # Create parent query
        p_res = await client.post(
            "/api/v1/parents", json={"id": user_id, "name": "PrefUser2", "email": "pref2@test.com"}
        )
        ...
        mapping = {"user_id": user_id, "role": "narrator", "voice_id": "narrator-female"}

        response = await client.post(f"/api/stories/{created_story_id}/voices", json=mapping)
