...
class TTSCache:
    ...
    def _generate_key(self, text: str, voice_id: str, options: Optional[Dict[str, Any]]) -> str:
        """Generate a stable hash key for the inputs."""
        # Sort keys for stability
        options_str = json.dumps(options or {}, sort_keys=True)
        content = f"{text}|{voice_id}|{options_str}".encode("utf-8")
        return hashlib.sha256(content).hexdigest()

    def get(self, text: str, voice_id: str, options: Optional[Dict[str, Any]] = None) -> Optional[bytes]:
    ...
    def set(self, text: str, voice_id: str, options: Optional[Dict[str, Any]], data: bytes) -> None:
        """Save audio to cache."""
        key = self._generate_key(text, voice_id, options)
